...
#define NUMBER_OF_FRAMES 128 //Total number of frames
#define PAGE_TABLE_SIZE 128 //Size of the page table
...
int main() {
    ...
    output = fopen("OUTPUT_128.csv","w+"); 
    //fprintf(output, "Logical Address, Physical Address, Value\n");
    while(fgets(buff, MAX_ADDRESS_LENGTH, addresses) != NULL) {
        totalnumber++;
        ...
        int framenumber = findframenumber(pagenumber);
        ...
        printf("%d\n",value);
        clock ++;
        fprintf(output, "%d,%d,%d\n",virtual, (framenumber * 256) + offsetnumber, value);
